...
if ($env:PT__installdir) {
  $AnsibleDir = Join-Path $env:PT__installdir "lib\puppet_x\ansible_modules\ansible_role_pip"
} else {
  # Fallback to Puppet cache directory
  $AnsibleDir = "C:\ProgramData\PuppetLabs\puppet\cache\lib\puppet_x\ansible_modules\ansible_role_pip"
}
...
$CollectionPlaybook = Join-Path $AnsibleDir "roles\paw_ansible_role_pip\playbook.yml"
...
if ((Test-Path (Join-Path $AnsibleDir "roles")) -and (Test-Path $CollectionPlaybook)) {
  ...
  $PlaybookDir = Join-Path $AnsibleDir "roles\paw_ansible_role_pip"
  ...
  $result = @{
    _error = @{
      msg = "playbook.yml not found in $AnsibleDir or $AnsibleDir\roles\paw_ansible_role_pip"
...
$ExtraVars = @{}
if ($env:PT_pip_package) {
  $ExtraVars['pip_package'] = $env:PT_pip_package
}
if ($env:PT_pip_executable) {
  $ExtraVars['pip_executable'] = $env:PT_pip_executable
}
if ($env:PT_pip_install_packages) {
  $ExtraVars['pip_install_packages'] = $env:PT_pip_install_packages
}
